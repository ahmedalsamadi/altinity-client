...
const Register = ({ isAuthenticated, register, showAlertMessage }) => {
  ...
  const onSubmit = (e) => {
    ...
    if (password !== password2) {
      showAlertMessage("Your confirm password is wrong", "error");
    } else {
        ...
        <div className="glass-strong rounded-2xl p-8 shadow-[0_0_50px_rgba(34,211,238,0.3)] border border-white/10">
          <div className="text-center mb-8">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-r from-cyan-400/20 to-fuchsia-500/20 border border-cyan-400/30 mb-4">
              <i className="fas fa-user-plus text-3xl bg-gradient-to-r from-cyan-400 to-fuchsia-500 bg-clip-text text-transparent"></i>
            </div>
            <h2 className="text-3xl font-bold bg-gradient-to-r from-cyan-400 to-fuchsia-500 bg-clip-text text-transparent mb-2">
              Sign Up
            </h2>
            <p className="text-white/70">Join the Altinity community</p>
          </div>
          ...
          <form onSubmit={onSubmit} className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium text-white/80 mb-2">
                <i className="fas fa-lock mr-2 text-fuchsia-400"></i>
                Confirm Password
              </label>
              <input
                type="password"
